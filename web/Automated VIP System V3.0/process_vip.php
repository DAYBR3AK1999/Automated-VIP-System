...
$vip_group = $_POST['vip_group'] ?? NULL;
$used = $_POST['used'] ?? '0';

$expire_date = $expire ? date('Y-m-d H:i:s', strtotime($expire)) : NULL;

$sql = "INSERT INTO sb_vip_system (code, name, steamid, expire, vip_group, used, added_by) VALUES (?, ?, ?, ?, ?, ?, ?)";
$stmt = $conn->prepare($sql);

if ($stmt) {

    $expire_date = ($expire !== '') ? date('Y-m-d H:i:s', strtotime($expire)) : NULL;
    $vip_group = ($vip_group !== '') ? $vip_group : NULL;

    $stmt->bind_param("sssssis", $code, $name, $steamid, $expire_date, $vip_group, $used, $added_by);
    $stmt->execute();
